...
from .common import toshl_client
...
def delete_entry(_id):
    ...
    logger.debug(url)
    toshl_client.delete(url)


def get_entries(page=0):
    return toshl_client.get(f'{TOSHL_URL}/entries',
...
def update_entry(entry):
    refreshed_entity = toshl_client.get(f'{TOSHL_URL}/entries/{entry["id"]}').json()
    entry['modified'] = refreshed_entity['modified']
    response = toshl_client.put(f'{TOSHL_URL}/entries/{entry["id"]}', json=entry)
...
def create_entry(entry):
    del entry['deleted']
    response = toshl_client.post(f'{TOSHL_URL}/entries', json=entry)
    logger.info(f'Done for {entry["id"]}')
    try:
        response.raise_for_status()
    except:
